...
class AiService {
  ...
  String _friendlyError(Object error) {
    ...
    if (normalized.contains('unsupporteduserlocation') ||
        normalized.contains('unsupported user location') ||
        normalized.contains('unsupported_user_location')) {
      return 'خدمة الذكاء الاصطناعي غير متوفرة في منطقتك الجغرافية حالياً (تتطلب الاتصال بـ VPN)';
    }
    ...
    if (normalized.contains('permission_denied') ||
      ...
      return 'تعذر المصادقة مع خدمة الذكاء الاصطناعي. تحقق من إعدادات Gemini.';
    }
    ...
    if (normalized.contains('resource_exhausted') ||
      ...
      return 'تم الوصول إلى حد استخدام الذكاء الاصطناعي مؤقتاً. حاول مرة أخرى لاحقاً.';
    }

    if (normalized.contains('deadline') ||
        normalized.contains('timeout') ||
        normalized.contains('timed out')) {
      return 'انتهت مهلة الاتصال بالذكاء الاصطناعي. تحقق من الإنترنت وحاول مرة أخرى.';
    }
    ...
    if (normalized.contains('socketexception') ||
      ...
      return 'تعذر الاتصال بخدمة الذكاء الاصطناعي. تحقق من الإنترنت وحاول مرة أخرى.';
    }
  ...
  Future<String> text(String prompt) {
    return _guard(() async {
      ...
      return response.text?.trim() ?? 'لم يتم الحصول على إجابة.';
    });
  }
  ...
  Future<List<String>> suggestTitles(String prompt) async {
    ...
    try {
      ...
      return raw
          .split('\n')
          .map((x) => x.replaceFirst(RegExp(r'^[-*\\d. ]+'), '').trim())
  ...
  Future<AiSummary> summarize(MediaItem i) async {
    final raw = await text(
      'Return JSON only with summary,strengths,audience,mood,similarTo. '
      'Analyze ${i.title}: ${i.overview}',
    );
    try {
      ...
      return AiSummary(
        ...
        audience: '${j['audience'] ?? ''}',
        mood: '${j['mood'] ?? ''}',
        similarTo: List<String>.from(j['similarTo'] ?? []),
      );
    } catch (_) {
      return AiSummary(
        summary: raw,
  ...
  Future<WorthWatching> worthWatching(MediaItem i) async {
    final raw = await text(
  ...
  Future<String> explainEnding(MediaItem i) =>
      text('Explain the ending of ${i.title}. Clearly label spoilers. ${i.overview}');

  Future<MoodResult> classifyMood(String q) async {
    final raw = await text(
      'Return JSON only with detectedMood,confidence,explanation,genres,avoidGenres,movies,anime. '
      'Mood: $q',
    );
    try {
      final j = jsonDecode(raw);
      return MoodResult(
        detectedMood: '${j['detectedMood'] ?? ''}',
        confidence: (j['confidence'] as num?)?.toDouble() ?? 0,
        explanation: '${j['explanation'] ?? ''}',
        genres: List<String>.from(j['genres'] ?? []),
        avoidGenres: List<String>.from(j['avoidGenres'] ?? []),
        movies: List<String>.from(j['movies'] ?? []),
        anime: List<String>.from(j['anime'] ?? []),
      );
    } catch (_) {
    ...
    return _guard(() async {
      ...
      final response = await visionModel.generateContent([
        Content.multi([
          ...
          TextPart(
            'Return JSON only with type,genres,audience,mood,colors,'
            'colorMeaning,symbols,verdict. Analyze this poster.',
          ),
        ]),
      ]);
  ...
  Future<TasteAnalysis> analyzeTaste(List<MediaItem> items) async {
    final data = items
        .take(30)
        .map((x) => '${x.title}|${x.genres.join(',')}|${x.userRating?.toString() ?? ''}')
        .join('\n');
    final raw = await text(
      'Return JSON only with archetype,personality,patterns,strengths,'
      'blindSpots,topGenres,avoidedGenres,recommendation. Library:\n$data',
    );
    try {
      final j = jsonDecode(raw);
      return TasteAnalysis(
        ...
        topGenres: List<String>.from(j['topGenres'] ?? []),
        avoidedGenres: List<String>.from(j['avoidedGenres'] ?? []),
